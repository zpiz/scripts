...
export default async function(ctx) {
  ...
  try {
    ...
    if (currentData && currentData.date) {
      ...
      if (ctx.storage) {
        ctx.storage.setJSON("cached_currency_current", currentData);
        ctx.storage.setJSON("cached_currency_yesterday", yesterdayData);
        ctx.storage.set("cached_currency_time", new Date().toISOString());
      }
    } else {
    ...
    if (ctx.storage) {
      try {
        const cachedCurr = ctx.storage.getJSON("cached_currency_current");
        const cachedYest = ctx.storage.getJSON("cached_currency_yesterday");

        if (cachedCurr && cachedYest) {
          currentData = cachedCurr;
          yesterdayData = cachedYest;
          isUsingCache = true;
        } else {
          isError = true;
        }
  ...
  if (family === "accessoryInline") {
    let txt = isError
      ? "获取失败"
      : listData.slice(0, 3).map(d => `${d.name.substring(0, 2)}${d.rate}${d.trendSymbol}`).join(" ");
    if (isUsingCache) txt += " ⚠️";
      ...
      children: [
        {
          ...
          children: [
            ...
            {
              type: "text",
              text: `${listData[0].rate}${listData[0].trendSymbol}${isUsingCache ? "⚠️" : ""}`,
    ...
    children: [
      {
        type: "text",
        text: item.name,
        font: { size: isSmallFont ? "footnote" : "subheadline", weight: "medium" },
        ...
        children: [
          {
            type: "text",
            text: item.rate,
            font: { size: isSmallFont ? "footnote" : "subheadline", weight: "bold" },
            textColor: colorMainText
          },
          {
            type: "text",
            text: item.trendSymbol,
            font: { size: isSmallFont ? "caption2" : "footnote", weight: "bold" },
            textColor: item.trendColor
          }
        ]
      }
    ]
  ...
  if (!isError) {
    if (isSmall) {
      ...
      listData.slice(half).forEach((item, index) => {
        ...
        if (index < listData.slice(half).length - 1) rightCol.push({ type: "spacer", length: rowSpacing });
      });
    ...
    contentChildren.push({
      type: "text",
      text: "网络请求失败且无可用缓存",
      textColor: colorTrendUp,
      font: { size: "subheadline" }
    });
  }

  let displayTime = new Date().toISOString();
  if (isUsingCache && ctx.storage) {
    const cachedTime = ctx.storage.get("cached_currency_time");
        ...
        children: [
          {
            ...
            text: timePrefixText,
            font: { size: "caption2" },
            textColor: isUsingCache ? "#FF9500" : colorSubText
          },
          {
            type: "date",
            date: displayTime,
