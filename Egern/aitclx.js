...
[Script]
# 同程旅行任务脚本 - 修复版
http-request ^https:\/\/app\.17u\.cn\/welfarecenter\/(index\/(signIndex|sign)|task\/taskList) script-path=tongcheng.js, tag=同程获取Token
*/
...
function getAccountIdentity(headers) {
  ...
  // 仅使用固定的 ID 作为唯一标识，不包含动态 Token
  // 优先级：MemberID > RefID > DeviceID
  const stableId = getHeader(headers, 'memberid') || 
                   saviorInfo.memberid || 
  ...
  return stableId || 'unknown_user';
}

// --- 以下为优化后的原脚本逻辑 ---
function getTodayDate() { return $.time('yyyy-MM-dd'); }
function getVersion() { return '11.3.8'; }
...
function normalizeHeaders(headers = {}, options = {}) {
  const next = { ...headers };
  const dropKeys = new Set(['host', 'content-length', 'accept-encoding', 'connection', 'traceparent']);
...
function getAccountName(account, index, total) {
  ...
  const saviorInfo = getSaviorInfo(headers);
  const memberId = getHeader(headers, 'memberid') || saviorInfo.memberid || '未登录';
  return `账号[${index + 1}] ${memberId.slice(-6)}`;
}

function saveAccountFromMitm(headers, signRequest = null) {
  let accounts = safeJsonParse(readData(KEY_SIGNHEADER), []);
  if (!Array.isArray(accounts)) accounts = [];
  
  const nextIdentity = getAccountIdentity(headers);
  const index = accounts.findIndex(acc => getAccountIdentity(acc.headers || acc) === nextIdentity);
  ...
  if (index > -1) {
    ...
    $.log(`更新已有账号: ${nextIdentity}`);
  } else {
    // 新增账号
    accounts.push(newAccount);
    $.log(`新增账号: ${nextIdentity}`);
  }

  $.setdata(JSON.stringify(accounts), KEY_SIGNHEADER);
  return { total: accounts.length, index: index > -1 ? index : accounts.length - 1 };
}

// --- 抓包控制逻辑 ---
if (typeof $request !== 'undefined') {
  const url = $request.url;
  if (url.includes('/welfarecenter/')) {
    const headers = $request.headers;
    let signRequest = null;
    if (url.includes('/index/sign')) {
      signRequest = { url: url, method: $request.method, headers: normalizeHeaders(headers, {keepVolatileHeaders:true}), body: $request.body };
    }
    const res = saveAccountFromMitm(headers, signRequest);
    $.msg('同程旅行', `获取Token成功`, `账号标识: ${getAccountIdentity(headers)}\n当前总账号数: ${res.total}`);
  }
  $.done();
} else {
  // 任务执行逻辑保持不变...
  $.log("脚本正常运行中，如需获取Token请进入同程旅行App-领福利界面");
  $.done();
}
